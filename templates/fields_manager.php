...
<ul class="nav nav-tabs">
  <li class="<?php echo $edit_one ? esc_attr('') : esc_attr('active') ?>">
    <a href="#tab-1"><?php esc_html_e('Created Fields Sections', 'onix-helper') ?></a>
  </li>
  <li class="<?php echo $edit_one ? esc_attr('active') : esc_attr('') ?>">
    <a
      href="#tab-2"> <?php echo $edit_one ? esc_html__('Edit ', 'onix-helper') . esc_html(sanitize_title($_POST['edit_fields_section'])) : esc_html__('Add new Fields Section', 'onix-helper') ?></a>
  </li>
</ul>
    ...
    <table class="wp-list-table widefat fixed striped table-view-list">
      ...
      foreach ($omb_list as $section) {
        ...
        <tr>
          ...
          <td><?php echo isset($section["description"]) ? esc_html($section["description"]) : esc_html('') ?></td>

...
class OhelperBaseCallbacks extends OhelperHtmlManager
{
  ...
  protected function omb_render_fields_box(array $args, string $base_content, $object_edit_mode, string $additional_content = '')
  {
    ...
    $tags = $this->get_common_escaping_params();
  ...
  function create_text_field_if_radio_true($args, $object_edit_mode)
  {
    ...
    return $this->render_text_field_if_radio_true("$option_name" . "[$name]", $value);
  }
  ...
  function render_text_field_if_radio_true($name, $value)
  {
    $box = '<div class="oh-hide-on-default"';
    $input_container = '<div data-depends-of="limited" class="oh-radio-block-to-show';

    $unlimited_params = '';
    $limited_params = 'data-show-if-active="limited" ';
    $numb_params = 'min="2" ';
    $numb_val = '';
    ...
    switch (true) {
      case $value > 1:
        ...
        $limited_params .= ' checked ';
        $numb_val = $value;
        break;
      case $value === '-1':
        $unlimited_params = ' checked ';
        $numb_params .= 'disabled';
        break;
      case ($value === null || $value === 1):
        $box .= ' style="display:none" ';
        $limited_params .= ' disabled ';
        $unlimited_params = ' disabled checked ';
        $numb_params .= ' disabled';
        break;
    }

    $unlimited = $this->render_radio_input($name, '-1', $unlimited_params, esc_html__('Unlimited', 'onix-helper'));
    $limited = $this->render_radio_input($name, '', $limited_params, esc_html__('Limited', 'onix-helper'));
    $numb = $this->render_number_input($name, $numb_val, $numb_params, esc_html__('Max section count', 'onix-helper'));

    $box .= ' > <div class="oh-radio-with-options manage-by-default-switcher">';
    $input_container .= '">';

    $field = $box . $unlimited . $limited . '</div> ' . $input_container . $numb . '</div></div>';
